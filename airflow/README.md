...
Three DAGs, one per medallion layer, chained via `ExternalTaskSensor` (not
one giant DAG) — so each layer can be backfilled or rerun independently
without forcing the others to rerun too:
...
warnings appeared, no import errors, and the task graph (22/8/2 tasks per
DAG) matched what was intended. This caught a real bug: `ExternalTaskSensor`
waiting on a whole external DagRun (no `external_task_id`) only accepts
`DagRunState` values for `failed_states` — `"upstream_failed"` is a
`TaskInstanceState` concept and isn't valid there.
